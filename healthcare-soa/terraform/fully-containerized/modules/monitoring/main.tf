  ...
  dashboard_body = jsonencode({
    widgets = [
      ...
      },
      {
        ...
        properties = {
          metrics = [
            for service in var.all_services : ["AWS/ECS", "CPUUtilization", "ServiceName", "${var.project_name}-${var.environment}-${service}", "ClusterName", var.ecs_cluster_name]
          ]
          view    = "timeSeries"
          stacked = false
      ...
      },
      {
        ...
        properties = {
          metrics = [
            for service in var.all_services : ["AWS/ECS", "MemoryUtilization", "ServiceName", "${var.project_name}-${var.environment}-${service}", "ClusterName", var.ecs_cluster_name]
          ]
          view    = "timeSeries"
          stacked = false
          region  = var.aws_region
...
resource "aws_cloudwatch_metric_alarm" "db_connection_count" {
  ...
  alarm_name          = "${var.project_name}-${var.environment}-${each.key}-connections-high"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = 2
  metric_name         = "MemoryUtilization"  # Use as a proxy for connection count 
  namespace           = "AWS/ECS"
  period              = 300
  statistic           = "Average"
  threshold           = 90
  alarm_description   = "Alarm when connection count is high for ${each.key} service"
