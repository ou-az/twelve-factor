# Script to fix the PostgreSQL connection in the product service
...
# Create a new ConfigMap with the correct PostgreSQL service name
Write-Host "Creating configuration with the correct PostgreSQL service name..." -ForegroundColor Cyan
$configYaml = @"
apiVersion: v1
kind: ConfigMap
metadata:
...
# Create a service for the new deployment
Write-Host "Creating service for the new deployment..." -ForegroundColor Cyan
kubectl expose deployment product-service-fixed -n ecommerce --port=8081 --target-port=8081 --name=product-service-fixed
...
# Verify PostgreSQL connectivity from inside the pod
...
if ($podName) {
    Write-Host "Running PostgreSQL connection test from pod $podName..."
    kubectl exec $podName -n ecommerce -- sh -c "wget -O- --timeout=5 postgres-service:5432 || echo 'Connection attempt made'"
}

Write-Host "Fix completed! Monitoring deployment logs..."
if ($podName) {
    kubectl logs $podName -n ecommerce --follow
}
